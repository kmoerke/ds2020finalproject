...
restrict it to only include Iowa and write it to another csv later. Here
is the source to the data.
...
![](README_files/figure-gfm/unnamed-chunk-3-3.png)<!-- --> The first
plot just shows population of a block to its walkability index. There is
...
compares intersection density to walkability score
...
shows distribution of each block by its walkability indexs weighted by
population
